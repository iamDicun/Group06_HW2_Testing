# Kế hoạch usability test U-01

- Ngày: 2026-08-01
- Website: https://eshop-sut-clone-ktpm-1.onrender.com/
- Flow: U-01
- FR: FR-05, FR-06
- Timebox: 10 phút/người
- Người điều phối: 23127459 - Huỳnh Vương Thụy Quân
- Thiết bị/trình duyệt test chính: 

## Mục tiêu

Đánh giá liệu người dùng có thể tự tìm kiếm sản phẩm, bấm chọn xem chi tiết và thêm sản phẩm đó vào giỏ hàng thành công hay không.

## Task scenario

> Giả sử bạn đang muốn tìm mua một chiếc iPhone trên trang web EShop. Bạn hãy tìm chiếc iPhone ưng ý nhất, thêm sản phẩm đó vào giỏ hàng và mở giỏ hàng ra để xác nhận đã có.

## Điều kiện

- Bắt đầu: trang chủ đã tải, chưa tìm kiếm, chưa chọn sản phẩm.
- Thành công: đã tìm thấy sản phẩm, xem chi tiết và thấy thông báo đã thêm vào giỏ hàng.
- Thất bại: bỏ cuộc, hết timebox, bị kẹt không phục hồi hoặc không đến được trạng thái thành công.
- Nếu sản phẩm "iPhone" không tồn tại: dùng từ khóa tìm kiếm khác và ghi deviation.

## Bảng người dùng tham gia

| Mã | Họ tên | Giới tính | Độ tuổi | Nghề nghiệp | Zalo/SĐT |
|----|--------|-----------|---------|-------------|----------|
| P01 | Nguyễn Thu Thảo | Nữ | 21 | Sinh viên | 056****415 |
| P02 | Huỳnh Vương Nhã Thụy | Nữ | 21 | Sinh viên | 077****699 |
| P03 | Huỳnh Thanh Hòa | Nam | 53 | Giảng viên | 0918***542 |
| P04 | Trần Minh Lý | Nam | 22 | Nhân viên L1 công ty manpower trong intel | 034***620 |
| P05 | Vũ Thành Đạt | Nam | 20 | Sinh viên | 098****064 |
| P06 | Nguyễn Ngọc Minh Châu | Nữ | 21 | Sinh viên | 081****205 |
| P07 | Vương Minh Kha | Nam | 33 | Nhân viên văn phòng | 093****394 |

## Checklist trước phiên

- [ ] Có đồng thuận tham gia/ghi hình nếu áp dụng.
- [ ] Dùng mã P01–P07, không ghi dữ liệu cá nhân không cần thiết.
- [ ] Kiểm tra website và đồng hồ bấm giờ.
- [ ] Chuẩn hóa start state: trang chủ https://eshop-sut-clone-ktpm-1.onrender.com/ đã tải hoàn toàn.
- [ ] Không tập trước flow cho người tham gia.
