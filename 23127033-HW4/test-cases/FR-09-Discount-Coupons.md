...
| :---: | :---: | :--- | :--- | :--- | :--- | :--- | :---: |
| `TC_CP_01` | Positive | Áp dụng mã giảm giá phần trăm hợp lệ (`SAVE10` - 10%) | Đã có sản phẩm trong giỏ, ở trang `/checkout` | `couponCode: "SAVE10"`, `totalAmount: 100000` | 1. Nhập mã `SAVE10`<br>2. Bấm "Áp dụng" | Hiển thị "Áp dụng mã thành công", số tiền tiết kiệm và tổng thanh toán mới giảm | P0 |
| `TC_CP_02` | Positive | Áp dụng mã giảm giá số tiền cố định hợp lệ | Đã có sản phẩm trong giỏ (tổng tiền > 50,000₫) | `couponCode: "FIXED50K"`, `totalAmount: 200000` | 1. Nhập mã `FIXED50K`<br>2. Bấm "Áp dụng" | Giảm trực tiếp 50,000₫ vào tổng thanh toán | P0 |
...
| `TC_CP_06` | Negative | Áp dụng mã giảm giá cho đơn hàng không đủ giá trị tối thiểu (min_order_amount) | Coupon yêu cầu đơn tối thiểu 500,000₫ | `couponCode: "VIP100K"`, `totalAmount: 200000` | 1. Nhập mã `VIP100K`<br>2. Click Áp dụng | Báo lỗi "Giá trị đơn hàng chưa đủ điều kiện áp dụng mã" | P1 |
...
| `TC_CP_09` | Negative | Người dùng dùng vượt quá số lần cho phép per user (`max_uses_per_user`) | User đã dùng mã này 1 lần trước đó | `couponCode: "ONETIMEONLY"` | 1. Đăng nhập user<br>2. Áp lại mã đã dùng | Báo lỗi "Bạn đã sử dụng hết số lần cho phép của mã này" | P1 |
...
| `TC_CP_12` | Edge | Áp dụng mã coupon loại phần trăm 100% (Miễn phí 100%) | Coupon discount_value = 100% | `couponCode: "FREE100"` | 1. Nhập `FREE100`<br>2. Click Áp dụng | Tổng tiền thanh toán giảm về 0₫ | P2 |
