// 23127033_Load_20260816.js — k6 Load Test Script
// Student ID: 23127033
...
const TEST_NAME = __ENV.TEST_NAME || '23127033_Load_20260816';
...
export const options = {
  ...
  thresholds: {
    http_req_duration: ['p(95)<1000'], // Provisional SLA: p95 latency under 1000ms
    http_req_failed: ['rate<0.05'],    // Error rate < 5%
    checks: ['rate>0.95'],            // Checks rate > 95%
  },
};
...
export default function () {
  ...
  group('01_Auth_Login', function () {
    ...
    if (!ok) {
      fail(`Login failed for ${user.email} (status ${res.status})`);
    }
  ...
  group('02_Auth_Profile', function () {
    ...
    check(res, {
      ...
      'profile email matches': (r) => r.json('email') === user.email,
    });
  });
  ...
  group('04_Transaction_ApplyCoupon', function () {
    ...
    const res = http.post(`${BASE_URL}/api/apply-coupon`, payload, {
      headers: { 'Content-Type': 'application/json' },
    });
    check(res, {
      'coupon status 200 or 400': (r) => r.status === 200 || r.status === 400,
    });
  });
  ...
  group('06_Transaction_Checkout', function () {
    const payload = JSON.stringify({
      total_amount: productPrice * 2 * 0.9, // 10% discount applied
      shipping_address: '123 Performance Testing St, HCM City',
    });
